...
## 2026-09-05 (later still) — Capo-aware tuning targets, and a pop-up tuner inside the chart

Two pieces of work on the tuner, committed separately so a Pages deploy fired once for each.

### Part 1 — capo-aware tuning targets

**Why.** Nicole had to take her capos off to tune, then put them back on and hope. A capo pulls the strings slightly sharp by pressing them down onto a fret, so a guitar tuned perfectly open can still be out in playing position. Tuning open is still the accurate baseline — that is what the strings are actually set to — but she needs to VERIFY in the position she is going to play in. She routinely plays with two capos at once.

**The model is the songbook's, reused rather than reinvented.** A full capo at fret 0-7 clamps all six strings; a cut capo covers A, D and G only and sits exactly two frets above whatever is under it (`cutFret = capo + CAPO_FRET`, CAPO_FRET imported from `src/cutcapo/tuning.js` rather than redefined). Both at once is the normal case.

**THE TRANSPOSE IS PER STRING, and that is the whole correctness question.** With both capos on the six strings do not share an offset: the three under the cut capo go up capo+2 while the other three go up capo. Nicole's usual capo 3 + cut is +3/+5/+5/+5/+3/+3, producing 98.00, 146.83, 195.99, 261.63, 293.66, 392.00 Hz. A single global offset would put three of the six on wrong targets and quietly send her to the wrong peg, which is worse than no capo support at all. Every string is computed from its OWN open frequency and its OWN semitone count; `TUNINGS` is never mutated, and with the capo off the function returns the untouched values so the open path cannot drift.

**The detection ceiling had to move with the capo, and this was the real bug.** `MAX_FREQ` is 360 Hz. High E is 370 Hz at full capo 2, 392 at 3, 440 at 5 — so from capo 2 upward that string sat OUTSIDE the search range and could never be detected however correctly it displayed. Demonstrated directly: a synthesised 392 Hz pluck is `NOT DETECTED` at ceiling 360 and detected at 391.94 Hz with clarity 0.986 at the derived ceiling of 428. `detectPitch` now takes an optional third argument defaulting to `MAX_FREQ`, so the open path is unchanged, and the ceiling is derived from the highest active target rather than being a second hardcoded constant that could fall out of step. `LOWPASS_HZ`, `LOWPASS_POLES`, `CLARITY_THRESHOLD` and `hasEnergyAt` are untouched — widening the range alone was sufficient, which is the outcome the brief asked to be reported either way. A cut capo alone needs no widening at all: it does not touch the high E and tops out at 220 Hz.

**Said plainly on screen.** With a capo set the readout reads `A2 → D3 · 146.83 Hz · capo 3 + cut`, the string buttons show the capoed note over its open name, and the panel says targets are capo positions, not open pitches, with "Tune open — that is the accurate baseline" when the capo is off. The tuner keeps its own capo setting; the songbook's stored per-chart capo is neither read nor written.

**Verified by actually running it.** `npx vite build` passes. All three required target lists were printed and match exactly — capo 3 + cut, capo 3 alone, and cut alone (82.41, 123.47, 164.81, 220.00, 246.94, 329.63, ringing E B E A B E with low E, B and high E untouched). Every capo 3 + cut target was fed to `detectPitch` as a synthesised harmonic-rich pluck and all six were detected, high E included. With the capo off the ceiling is exactly `MAX_FREQ` and every open target still detects, as do all four bass4 and all five bass5 strings. In the browser the panel renders, capo 3 + cut shows "cut capo on · fret 5 (A D G)", the string buttons read G2/E2 D3/A2 G3/D3 C4/G3 D4/B3 G4/E4, the diagnostic Range row moves from 28–360 to 28–428 Hz and back when the capo is cleared.

### Part 2 — pop-up tuner inside the chart view

**Why.** On stage a guitar drifts out of tune mid-set, and the only way to fix it was to back out of the chart entirely, cross to the tuner screen, tune, then navigate back and find your place. That is unacceptable during a service.

**The tuner is reused, not rebuilt.** `src/tuner/Tuner.jsx` is imported as-is with its own mic handling, pitch detection, headstocks, styles and everything Part 1 added. A "🎯 Tune" button in the chart toolbar (44px tall, the touch minimum) opens it in a modal OVER the chart. The chart is never unmounted — that is the entire point, and it is why closing lands on the same song, same page, same scroll position, same capo and key with nothing reloading.

**Releasing the microphone is structural, not a cleanup step.** Closing unmounts `Tuner`, whose existing unmount effect calls `TunerAudio.stop()`, which stops every MediaStream track and closes the AudioContext. Because the release rides on unmount rather than on a handler, there is no path that closes the modal without releasing the mic.

**The iOS gesture chain.** The tuner needs a real user gesture to resume its AudioContext (commit d234424). The button's `onClick` mounts the tuner synchronously, so `startAudio` runs inside the gesture's own task rather than after an await — the diagnostic's Audio row reads "running" when the modal opens, which is the check that the chain is intact.

**Gating** is `instrument !== "keys"`, character-identical to the existing tuner button's condition in `App.jsx`, with `instrument` threaded App → Songbook → ChartView. Teacher passes "guitar" and sees it.

**Verified by actually running it.** `npx vite build` passes. In real Chrome as Teacher: opened Holy Forever from the set at position 2/4, set capo 2 + cut ("Key: C - Capo 2 (A#) + cut capo (fret 4)"), opened the tuner from the toolbar and closed it — the chart came back with the same 6 blocks, the same key/capo header, `songbook_capo_holy-forever-english` still `{"capo":2,"cut":true}`, the same scroll position and the same PREV/NEXT bar. The tuner's diagnostic read Audio: running inside the modal. Closing removes `.tn-shell` from the DOM entirely, which is what invokes the mic teardown. The inactivity timeout was checked by hooking `setInterval`: ChartView registers its 60s keepalive as ids 37 and 38 on chart open, and after opening the modal the registered ids are still exactly 37 and 38 — no new interval, so ChartView never unmounted and the keepalive keeps firing while the tuner is open. At a 1194×648 viewport the modal sheet measures 560×596, fits both axes, scrolls internally to 400px while the page underneath stays at scrollY 0, and produces no horizontal page scroll.

### Caveats — please read before trusting the mic behaviour

**The microphone was never actually granted in this environment, so the mic-release step was NOT verified end to end.** Chrome under automation left `navigator.permissions.query({name:'microphone'})` at `prompt`, `enumerateDevices()` returned zero labelled audio inputs, and the tuner sat at "Starting the microphone…" — `getUserMedia` never resolved. What was verified is that closing the modal unmounts `Tuner`, and separately by reading `audio.js` that its unmount path calls `stop()`, which calls `t.stop()` on every track and closes the context. That is a sound chain, but it is code reading plus a DOM check, NOT an observed track transition to `ended`. This project's tuner bugs have all been found on real hardware and none by reading code, so this specific claim should be confirmed on the iPad: open the tuner from a chart, close it, and check the browser's recording indicator goes out.
